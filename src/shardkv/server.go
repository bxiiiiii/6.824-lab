	...
	if kv.LastConfig.Num > args.ConfigNum {
		...
		for _, v := range args.ShardsNum {
			...
			if args.Sender == kv.rqShardRecord[v].Receiver && args.ConfigNum == kv.rqShardRecord[v].ConfigNum {
				...
				// if kv.rqShardRecord[v].Receiver != args.Sender&&kv.sha{
				// 	reply.Err = OK
				// 	replyShard.Status = "NotExist"
				// }
			}
...
func (kv *ShardKV) Apply() {
	for entry := range kv.applyCh {
		if entry.CommandValid {
			...
			case "RequireShard":
				DEBUG(dError, "S%v[shardkv][gid:%v] before apply rqs %v %v %v %v", kv.me, kv.gid, kv.LastConfig.Num, op.ConfigNum, kv.shards[op.ShardsNum[0]].Status, kv.rqShardRecord[op.ShardsNum[0]].Receiver)
			...
			var istem bool
			var isSend bool
			temShards := make(map[int]Shard)
			if d.Decode(&applyIndex) != nil || d.Decode(&shards) != nil || d.Decode(&rqRecord) != nil || d.Decode(&curConfig) != nil || d.Decode(&lastConfig) != nil || d.Decode(&rqShardRecord) != nil || d.Decode(&isWorking) != nil || d.Decode(&isChanged) != nil || d.Decode(&istem) != nil || d.Decode(&temShards) != nil || d.Decode(&isSend) != nil {
				panic("decode fail")
			} else {
			...
			if !kv.isSend {
				...
				for k, v := range kv.LastConfig.Shards {
					if v == kv.gid {
						shard := Shard{
							ShardNum: temShards[k].ShardNum,
							Storage:  make(map[string]string),
							RqRecord: make(map[int64]RequestInfo),
							Status:   temShards[k].Status,
						}
						shard.Storage = temShards[k].Storage
						shard.RqRecord = temShards[k].RqRecord
						kv.shards[k] = shard
						if shard.Status == "Working" {
							kv.rqShardRecord[k].Receiver = -1
						}
			...
			for _, shardNum := range need2Control {
				for kv.temShards[shardNum].Status != "Working" && kv.temShards[shardNum].Status != "WorkingWaitForAdd" && kv.temShards[shardNum].Status != "Failed" {
					...
					for k, v := range kv.temShards {
						DEBUG(dLog, "S%v [shardkv][gid:%v] temStatus: %v %v %v", kv.me, kv.gid, k, v.Status, kv.shards[k].Status)
					}
					kv.cond.Wait()
				}
...
func (kv *ShardKV) Timer() {
	...
	for {
		...
		// if !kv.isWorking && kv.LastConfig.Num == kv.curConfig.Num {
		if kv.istem {
			if kv.curConfig.Num < kv.LastConfig.Num {
				operation := Op{
					Type:   "ConfigChange",
					Index:  nrand(),
					Config: kv.LastConfig,
