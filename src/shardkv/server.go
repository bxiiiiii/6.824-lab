...
func (kv *ShardKV) HandleRequireShard(args *RequireShardArgs, reply *RequireShardReply) {
	...
	if kv.LastConfig.Num > args.ConfigNum {
		...
		// DEBUG(dCommit, "S%v[shardkv][gid:%v] RequireShard is failed, conNum:m-o:%v-%v", kv.me, kv.gid, kv.LastConfig.Num, args.ConfigNum)
		reply.Err = ErrorTimeDeny
		kv.mu.Unlock()
		return
	}
	...
	for k, v := range kv.rqRecord {
		if k == args.Index {
			...
			} else if v.Status == ErrorTimeDeny {
				break
			}
			DEBUG(dInfo, "S%v [shardkv][gid:%v] handle same rq %v %v %v", kv.me, kv.gid, args.Index, args.ShardsNum, reply)
			kv.mu.Unlock()
			return
		}
	}
	...
	DEBUG(dLog2, "S%v[shardkv][gid:%v] RequireShard %v %v", kv.me, kv.gid, i, args.ShardsNum)
	...
	if kv.rqRecord[args.Index].Status == ErrorOccurred {
		...
		for _, v := range args.ShardsNum {
			if kv.LastConfig.Num > args.ConfigNum {
				...
				if !kv.isWorking {
					DEBUG(dCommit, "S%v[shardkv][gid:%v] not changed ,RequireShard is failed, conNum:m-o:%v-%v", kv.me, kv.gid, kv.curConfig.Num, args.ConfigNum)
					reply.Err = ErrorTimeDeny
					delete(kv.rqRecord, args.Index)
					kv.mu.Unlock()
					return
...
func (kv *ShardKV) Apply() {
	for entry := range kv.applyCh {
		if entry.CommandValid {
			...
			if d.Decode(&applyIndex) != nil || d.Decode(&shards) != nil || d.Decode(&rqRecord) != nil || d.Decode(&curConfig) != nil || d.Decode(&lastConfig) != nil {
				panic("decode fail")
			} else {
...
func (kv *ShardKV) ApplyConfigChange(lastConfig shardctrler.Config) {
	...
	for shardNum, gid := range lastConfig.Shards {
		if kv.gid == gid {
			...
			if kv.curConfig.Shards[shardNum] != 0 {
			...
			for si := 0; si < len(servers); si++ {
				...
				if ok {
					...
					if reply.Err == ErrorConfigOutOfDate {
						...
						DEBUG(dLog2, "S%v[shardkv][gid:%v] RequireShard failed %v", kv.me, kv.gid, shardsNum)
						kv.mu.Unlock()
						kv.cond.Broadcast()
						return
					}
...
func (kv *ShardKV) UpdateShard(shardNum []int) {
	kv.mu.Lock()
	shards := make(map[int]Shard)
	for _, shardnum := range shardNum {
		shard := Shard{
			...
			Storage:  make(map[string]string),
			RqRecord: make(map[int64]RequestInfo),
			Status:   kv.shards[shardnum].Status,
		}
		shard.Storage = kv.shards[shardnum].Storage
		shard.RqRecord = kv.shards[shardnum].RqRecord
